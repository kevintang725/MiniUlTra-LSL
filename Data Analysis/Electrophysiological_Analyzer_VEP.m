...
%% Import File
filename.data = "/Users/KevinTang/Desktop/UTAustin/Code/GitHub/Human Trials/FUS (Visual Evoked Potential)/Kevin_10022023/FUS+VEP+_1.csv";
...
figure
subplot(2,1,1)
plot(time*1e-3,filtered_data(:,channel));
hold on
plot(time*1e-3 - triggerdelay*1e-3,data(:,58)*max(filtered_data(:,channel)),'r','LineWidth',1);
hold off
xlabel('Time (s)')
ylabel('Amplitude (uV)')
axis([10 25 -inf inf])
...
xline(-(triggerdelay+50), '--r', 'LineWidth',2)
...
function epochs = extract_epochs(filtered_data, fs, epoch_triggers)
    ...
    for k = 1:width(filtered_data) - 1
        for i = 1:length(locs)
            e = filtered_data(locs(i)-(LB/(1/fs)/1000):locs(i)+(UB/(1/fs)/1000),k);
            epochs{k}(:,i) = e;
        end
    end
end
